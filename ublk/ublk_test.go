...
// ---------------------------------------------------------------------------
// Test: backend errors on some IOs don't kill the device.
// After a failed IO, subsequent IOs to good offsets must still work.
// ---------------------------------------------------------------------------
...
func TestBackendErrorRecovery(t *testing.T) {
	...
	const size = 2 * 1024 * 1024
	const badOff = 512 * 1024 // this offset triggers errors
	...
	// Write to the bad offset — this should fail (or be cached and fail on sync).
	badBuf := make([]byte, 4096)
	unix.Pwrite(fd, badBuf, badOff)

	// Write to a good offset — this MUST succeed. If the worker crashed
	// from the previous error, this would hang or fail.
	goodBuf := make([]byte, 4096)
	for i := range goodBuf {
		goodBuf[i] = 0xAB
	}
	n, err := unix.Pwrite(fd, goodBuf, 0)
	if err != nil || n != len(goodBuf) {
		t.Fatalf("write to good offset after bad: n=%d err=%v", n, err)
	}

	// Verify the good write landed in the backend.
	got := make([]byte, 4096)
	backend.mu.RLock()
	copy(got, backend.data[0:4096])
	backend.mu.RUnlock()
	if !bytes.Equal(got, goodBuf) {
		t.Error("good write after bad write did not reach backend")
	}
}
