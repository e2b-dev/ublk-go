...
// A backend without Discarder must reject BLKDISCARD. BLKZEROOUT isn't tested
// here because the kernel silently falls back to regular WRITE when
// WRITE_ZEROES is unadvertised.
func TestBackendWithoutDiscarderRejectsDiscard(t *testing.T) {
	...
	if err := blkRange(t, fd, unix.BLKDISCARD, 0, 4096); err == nil {
		t.Fatal("BLKDISCARD unexpectedly succeeded without Discarder")
	}
}
...
func TestBlkDiscardAndZeroOut(t *testing.T) {
	...
	for _, r := range []struct {
		name string
		off  int64
	}{{"BLKDISCARD", 0}, {"BLKZEROOUT", 2 << 20}} {
		got := make([]byte, 1<<20)
		backend.ReadAt(got, r.off)
		if !bytes.Equal(got, zero) {
			t.Fatalf("%s: range not zeroed", r.name)
		}
	}
}
